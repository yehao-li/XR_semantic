...
def calc_sum_SemCom(user_list):
    ...
    for user in user_list:
        if user.is_semantic:
            ...
            bitcom = calc_BitCom(user, user_list)
...
class XR:
    ...
    def alloc_power_avg(self, user_list):
        for user in user_list:
            user.power = P_d / self.user_num

    # select semantic users from user list
    def select_semantic_user(self, user_list):
        semantic_user_num = int(self.user_num * self.semantic_ratio)
        for user in user_list:
            user.sinr = calc_sinr(user, user_list)
        user_list.sort(key=lambda x: x.sinr)
        for i in range(semantic_user_num):
            user_list[i].is_semantic = True
